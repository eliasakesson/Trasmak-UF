...
export default function Feedback() {
	...
	function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
		...
		const designRef = ref(db, `feedback/${Date.now()}`);
	...
	return (
		<main className="mx-auto flex min-h-[calc(100vh-108px)] w-full max-w-7xl flex-col items-center space-y-16 px-8 py-16">
			<div className="flex w-full flex-col items-center gap-6 border-b pb-8">
				<MdFeedback className="text-6xl text-primary" />
				<h1 className="text-center text-4xl font-bold text-gray-900">
					Feedback
				</h1>
				<span className="max-w-prose text-center text-muted">
					Har du hittat en bugg eller har förslag på förbättringar?
					Skicka in din feedback här så tar vi en titt på det. Tack
					för att du hjälper oss att bli bättre!
				</span>
			</div>
			<form
			...
			>
				...
				/>
				<Checkbox
					label={`Meddela mig med svar${user ? `, till { ${user.email} }` : ""}`}
					name="notify"
